...
GLuint loadProjPts(const char * imagepath){
	...
	if (!pFile)							    {printf("%s could not be opened.\n", imagepath); getchar(); return 0;}
    // obtain file size:  
	...
	if (!buffer)							    {printf("fail to allocate buffer.\n"); return 0;}
	// copy the file into the buffer:
	result = fread (buffer,1,lSize,pFile);
	if (result != lSize)							    {printf("fail to read the file.\n"); return 0;}
